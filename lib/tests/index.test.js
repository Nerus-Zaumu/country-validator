...
test('() and (5) should yield arrays of unequal lengths of all countries', () => {
    ...
    expect((0, __1.getAllCountries)(5)).toEqual(data_1.countries.slice(4));
});
test('Depending on the params passed, there should be a sorted list of countries received', () => {
    ...
    expect((0, __1.sortCountries)({ data: africanCountries, limit: 5 })).toEqual(africanCountries.sort(compare_util_1.compareFunc).slice(4));
    expect((0, __1.sortCountries)({ limit: 5 })).toEqual(data_1.countries.sort(compare_util_1.compareFunc).slice(4));
